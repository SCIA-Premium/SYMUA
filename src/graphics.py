...
from core.vector2D import normalize, unit_vector, length
...
def shapes(geom, **kargs):
    ...
    if isinstance(geom, LineString):
        ...
        raise TypeError('Unknown geometry type: {}'.format(type(geom)))
...
class MultiAgentPlot(pg.PlotItem):
    ...
    def agents(self, agents):
        if is_model(agents, 'circular'):
            ...
            self.__agents.addItem(self)
            self.__agents.setData(agents)
        elif is_model(agents, 'three_circle'):
            self.__agents = ThreeCircleAgents(agents)
            self.__agents.addItem(self)
            self.__agents.setData(agents)
        else:
            raise TypeError('Unknown model: {}'.format(agents.model))
